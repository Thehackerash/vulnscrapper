...
function startMonitoring() {
  ...
  cronUpdate.start();
  console.log(
    `Monitoring started with an interval of ${cronTime.interval} minute(s).`,
  );
}
...
function listSites() {
  ...
  sitesToMonitor.forEach((site, index) => {
    console.log(
      `${index + 1}. ${site.url} (CSS Selector: ${site.css}, Last Updated: ${site.lastUpdated})`,
    );
  });
}
